...
export const createCategory = asyncHandler(async (req, res, next) => {
	...
	if (!name || !icon || !color) return next(createError("Please enter all fields !", 400))
...
export const approveOrRejectVendors = asyncHandler(async (req, res, next) => {
	...
	if (action == "true") {
		...
		if (!!!data) return next(createError("No such request found for vendor!", 403))
		const { _id, user } = data
		...
		return next(createError("Invalid request, please provide action.", 403))
	}
})
